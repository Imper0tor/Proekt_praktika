...
class Interface():
    ...
    def __init__(self, root, pm):
        ...
        self.pm = pm
        self.frame_top = Frame(self.root,  width=1024, height=35, bg='green').grid(columnspan=8, sticky='we')

        self.bttn1 = Button(self.frame_top, width=15, text='Создать папку',command=lambda: self.lb_trigger(self.add_dir_trigger))
        self.bttn1.grid(row=0, column=1, sticky='w', padx=5)
        self.bttn2 = Button(self.frame_top, width=15, text='Создать файл',command=lambda: self.lb_trigger(self.add_file_trigger))
        self.bttn2.grid(row=0, column=2, sticky='w', padx=5)
        self.bttn3 = Button(self.frame_top, width=15, text='Переименовать', command=lambda: self.lb_trigger(self.name_change))
        self.bttn3.grid(row=0, column=3, sticky='w', padx=5)
        self.bttn4 = Button(self.frame_top, width=15, text='Удалить', command=lambda: self.lb_trigger(self.delete_item))
        self.bttn4.grid(row=0, column=4, sticky='w', padx=5)
        self.bttn5 = Button(self.frame_top, width=15, text='Копировать', command=lambda: self.lb_trigger(self.get_copy_item))
        self.bttn5.grid(row=0, column=5, sticky='w', padx=5)
        self.bttn6 = Button(self.frame_top, width=15, text='Вырезать', command=lambda: self.lb_trigger(self.get_cut_item))
        self.bttn6.grid(row=0, column=6, sticky='w', padx=5)
        self.bttn7 = Button(self.frame_top, width=15, text='Вставить', command=lambda: self.lb_trigger(self.paste_item))
        self.bttn7.grid(row=0, column=7, sticky='w', padx=5)

        self.frame_middle = Frame(self.root)
        self.frame_middle.grid(columnspan=8)

        self.lb_1 = Listbox(self.frame_middle, width=85,height=40)
        self.lb_1.bind('<Double-Button-1>', lambda _: self.pm.double_click(self.lb_1))
        self.lb_1.grid(row=2, column=0,  sticky='w')

        self.lb_2 = Listbox(self.frame_middle, width=85,height=40)
    ...
    def create_edit_spot(self, *args):
        ...
        self.edit_spot = Entry(bottom_frame, textvariable=v,width=40)
        if len(args) == 3:
            item = args[2]
            self.edit_spot.insert(0, item)
        lb, f = args[0], args[1]
        self.edit_spot.grid(row=2, column=1, columnspan=4, padx=50, sticky='e')

        self.bttn_apply = Button(bottom_frame, text='Ок',width=6,
        command=lambda: f(lb, self.edit_spot.get(), self.del_edit_spot, self.refresh_lb, item))
        self.bttn_apply.grid(row=2, column=3, sticky='e', columnspan=2,)
        self.bttn_cancel = Button(bottom_frame, text='Отмена', command=lambda: self.del_edit_spot())
        self.bttn_cancel.grid(row=2, column=5, sticky='w', columnspan=2)
